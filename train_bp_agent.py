...
def start_tensorboard(log_dir="./runs", port: int = 6006):
    ...
    try:
        ...
        print("[!] TensorBoard not found. Make sure it's installed: pip install tensorboard")
        return None
...
def train(config_path: str = None, **override_kwargs):
    ...
    samples_per_epoch = override_kwargs.get("samples_per_epoch", training_cfg.get("samples_per_epoch", 1024))
    use_tensorboard = override_kwargs.get("use_tensorboard", training_cfg.get("use_tensorboard", True))
    ...
    if use_tensorboard:
        if log_dir is None:
            log_dir = os.path.join("runs", tb_log_prefix + datetime.now().strftime("%Y%m%d-%H%M%S"))
        ...
        for batch_idx in pbar:
            ...
            for rollout in rollouts:
                ...
                for idx in valid_indices:
                    ...
                    logits = logits + mask
                    probs = torch.softmax(logits, dim=-1)
                    new_log_probs_list.append(torch.distributions.Categorical(probs).log_prob(actions[len(new_log_probs_list)]))
            ...
            if writer is not None:
                ...
                writer.add_scalar("Loss/total", total_loss / ((batch_idx + 1) * actual_rollout_steps), global_step)
                writer.add_scalar("Loss/kl_divergence", batch_kl / len(rollouts), global_step)
                writer.flush()  # 确保数据立即写入磁盘

            pbar.set_postfix({"Loss": f"{total_loss / ((batch_idx + 1) * actual_rollout_steps):.4f}"})
