...
class PlayerHeroEncoder(nn.Module):
    ...
    def __init__(
        self, num_heroes=NUM_HEROES, hidden_dim=128, embed_dim=64, nhead=4, num_layers=2
    ):
        super().__init__()
...
class WinRateOracle(nn.Module):
    def __init__(
        ...
        # 8. 预计算所有英雄的静态特征
        # Handle device correctly - existing tensors might already be on different devices
        ...
        if use_text:
            # Get device from an existing tensor in HERO_ID_SEMANTIC_MAP
            first_hero_sem = next(iter(HERO_ID_SEMANTIC_MAP.values()))
            default_device = first_hero_sem.device
            # Don't move to DEVICE here - will be moved with model to device
            self.register_buffer(
                "all_hero_sem",
                torch.stack(
                    [
                        HERO_ID_SEMANTIC_MAP.get(
                            hero_id, torch.zeros(1024, device=default_device)
                        )
                        for hero_id in range(1, NUM_HEROES + 1)
                    ]
                ),
    ...
    def forward(
        ...
        r_emb = self.hero_encoder(
            radiant_hero_ids, radiant_hero_attrs, radiant_hero_semantics
        )
        ...
        if self.use_player_heroes:
            ...
            combined_feat = torch.cat(
                [cls_feature, r_player_feat, d_player_feat], dim=-1
            )
        else:
    ...
    def hero_input_from_ids(self, hero_ids: torch.Tensor):
        ...
        # 获取属性 [5, F] 或 [B, 5, F] - 确保all_hero_attrs在正确设备上
    ...
    def predict(
        ...
        with torch.no_grad():
            pred = self.forward(
                r_ids, r_attrs, r_sem, d_ids, d_attrs, d_sem, r_player, d_player
            )
    ...
    def _process_player_feats(self, player_feats, device=None):
        ...
        if not isinstance(player_feats, torch.Tensor):
            player_feats = torch.tensor(
                player_feats, dtype=torch.float32, device=device
            )
        else:
    ...
    def load_player_hero_data(
        self, match_id: int, data_file: str = "./data/player_hero_features.json"
    ):
        """
        ...
        radiant_feats = torch.tensor(
            match_data["radiant_player_features"], dtype=torch.float32
        )
        dire_feats = torch.tensor(
            match_data["dire_player_features"], dtype=torch.float32
        )
...
class OracleTrainingDataset(torch.utils.data.Dataset):
    ...
    def _build_player_features(self, players):
        ...
        for player in players[:5]:
            ...
            for hero_id, games, winrate in hero_list[: self.max_heroes_per_player]:
                # 只添加实际存在的英雄
                if (
                    hero_id in valid_hero_ids
                    and hero_id <= NUM_HEROES
                    and games >= self.min_games_per_hero
                ):
    ...
    def __getitem__(self, idx):
        ...
        r_player_feats = torch.tensor(
            self._build_player_features(radiant_players), dtype=torch.float32
        )
        d_player_feats = torch.tensor(
            self._build_player_features(dire_players), dtype=torch.float32
        )
