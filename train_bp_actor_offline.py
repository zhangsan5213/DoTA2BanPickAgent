...
class BP_PPOTrainer:
    def __init__(self, agent: PPODecoderAgent, oracle: WinRateOracle, writer: SummaryWriter, lr=1e-4, gamma=0.95, lam=0.95, eps_clip=0.2, ent_coef=0.01, critic_loss_factor=1):
        self.agent = agent
        self.oracle = oracle  # WinRateOracle
        self.optimizer = torch.optim.Adam(agent.parameters(), lr=lr, weight_decay=0.01)
        self.gamma = gamma
        self.lam = lam
    ...
    def prep_data(self, batch):
        ...
        return device, hero_seq, team_seq, type_seq, r_picks, d_picks
    ...
    def compute_reward(self, radiant_picks, dire_picks):
        ...
        with torch.no_grad():
            ...
            win_prob = self.oracle.predict(radiant_picks, dire_picks)
            # 映射到 -1 ~ 1
            reward = (win_prob - 0.5) * 2
            return reward
    
    def train_step_critic_pretrain(self, batch):
        device, hero_seq, team_seq, type_seq, r_picks, d_picks = self.prep_data(batch)
        
        # 1. 计算最终奖励 R (假设是 0 或 1)
        with torch.no_grad():
            # final_reward 范围 [-1, 1]
            final_reward = self.compute_reward(r_picks, d_picks) # [B]
            ...
            weights = 0.95 ** (T - 1 - steps) # [24]
            
            # 3. 计算 Soft Targets
            # targets 形状: [B, 24]
            targets = final_reward * weights
    ...
    def train_step(self, batch, num_ppo_iters: int = 4, global_step: int = 0):
        device, hero_seq, team_seq, type_seq, r_picks, d_picks = self.prep_data(batch)
        ...
        # 2. 计算奖励序列 (只有最后一步有值)        
        rewards = self.compute_reward(r_picks, d_picks).flatten()
        ...
        self.writer.add_scalar('Batch/Actor Loss', actor_loss.item(), global_step)
        self.writer.add_scalar('Batch/Critic Loss', critic_loss.item(), global_step)
...
if __name__ == "__main__":
    ...
    trainer = BP_PPOTrainer(agent, oracle, writer, lr=1e-4, critic_loss_factor=1)
    ...
    for epoch in range(epochs_together):
        ...
        for batch in pbar:
            actor_loss, critic_loss = trainer.train_step(batch, global_step=global_step)
            pbar.set_postfix({"Actor loss": f"{actor_loss:.4f}", "Critic loss": f"{critic_loss:.4f}"})
            global_step += 1